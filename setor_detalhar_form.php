...
<html lang="pt-br" style="height: 100%; min-height: 100%;">
    ...
    <section class="container text-center text-light" style="margin: auto;">
         ...
         <div class="bg-dark col m-1 text-dark" style="background-image: url('img/back05.jpg'); background-position: center;  background-repeat: no-repeat; background-size: cover;">
                        <label class="titulorel pt-2" style="font-weight: bold; color:white; font-weight: bold; text-shadow: 2px 2px 4px black; font-size: 28px;"> Detalhar Setor</label>
                        <form name="pontuar" action="setor_detalhar.php" method="POST" class="pt-3">
                        
                                    <label class="labelform"> Setor </label>
                                    <select class="parametrosform2" name="id_setor" style="text-align: center;"> 
                                        <?php
                                            $lista_setor = "SELECT s.id_setor as id_setor, s.setor as setor from setor as s;";
                                            $resultado_lista_setor = mysqli_query($conexao,$lista_setor);
                                            while($row_lista_setor = mysqli_fetch_assoc($resultado_lista_setor)){ ?>
                                                <option> <?php echo $row_lista_setor['id_setor']; echo " - "; echo $row_lista_setor['setor'];?>
                                                </option> <?php
                                            }
                                            ?>                                         
                                    </select><br><br>
