...
<html lang="pt-br" style="height: 100%; min-height: 100%;">
    ...
    <section class="container text-center text-light" style="margin: auto;">
         ...
         <div class="container">
             ...
             <h1 class="pb-4"style="font-size: 30px; color: white; padding: 20px; font-weight: bold; text-shadow: 2px 2px 4px black;"> Partidas </h1>
    ...
    <?php
            ...
            echo "<th>#</th>";
            echo "<th>Rodada</th>";
            echo "<th>id_partida</th>";
            echo "<th>Nome_Partida</th>";
            echo "<th>Clube_01</th>";
            echo "<th> X </th>";
            echo "<th>Clube_02</th>";
            echo "<th>Status</th>";
            ...
            $sql = "SELECT
            (select r.nome_rodada from rodadas as r where r.id_rodada = p.id_rodada) as Rodada,
            p.id_partida as id_partida,
            p.nome_partida as Nome_Partida,
            (select t.nome from times as t where t.id_time = p.id_time_01) as Clube_01,
            (select t.nome from times as t where t.id_time = p.id_time_02) as Clube_02,
            p.partida_data as partida_data
            ...
            while ($registro = mysqli_fetch_array($resultado))
            {
                ...
                $Nome_Partida = $registro['Nome_Partida'];
                $Clube_01 = $registro['Clube_01'];
                $Clube_02 = $registro['Clube_02'];
                $partida_data = $registro['partida_data'];

                if($partida_data < $hoje){
                    $status = 'Encerrada';
                }else{
                    $status = 'Andamento';
                }
                ...
                echo "<td>".$cont."</td>";
                echo "<td>".$Rodada."</td>";
                echo "<td>".$id_partida."</td>";
                echo "<td>".$Nome_Partida."</td>";
                echo "<td>".$Clube_01."</td>";
                echo "<td>".$X."</td>";
                echo "<td>".$Clube_02."</td>";
                echo "<td>".$status."</td>";
